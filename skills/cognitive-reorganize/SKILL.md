...
扫描范围（v1.1 扩展为全工作区，按优先级）：
  0. 认知结构/ 现有状态（第零步扫描结果）
  1. 项目产品定义目录/
  2. cognitive/L2_fragments/reflections/ 等碎片子目录
  3. 项目论文目录（不属于认知结构，归入 Loop 3）
  4. 执行规范目录（Loop 1 文档，不迁移，但记录）
  5. Skill 体系设计目录（Loop 1 设计工件，记录归属）
...
### 第五步【γ】：生成正式分类清单（文档归属涉及认知架构决策，须用户确认）

```
Write: 认知结构/文档分类清单.md【α】（分类清单文件本身 AI 直接写入）
...
### 第七步【α】：Phase 2 — 迁移（用户已在第五步确认，AI 直接执行8步协议）
...
Loop1 文档（开发规范/踩坑速查/Skill模板等）：
  → 若在认知结构内 → 迁出到执行规范目录 或 对应 Skill 知识库
  → 若已在正确位置 → 标注 ✅ 无需迁移
  → 若是「引用模板有REF-EXT价值」→ 在对应 L1 维度知识库里放一份（保持 .cursor/ 里的原本）

Loop3 文档（项目工作文档）：
  → 若属于项目工作产物 → 确认归属项目，迁入对应项目目录
  → 若已在 项目群/ 里 → 标注 ✅ 无需迁移
...
**本次执行产出**：K-object 归属清单（L0大脑总地图.md 更新）+ S-object 操作记录（系统日志）
- 文档归属清单（K-object，已写入 L0 大脑总地图相关章节）
- 移动/归档操作记录（S-object，追加到系统日志）

**产出位置**：`cognitive/L0_brain_map.md`（更新）

**Loop 路由**：
- **通路B（Loop 3 → Loop 2）**：
  → 每次重组后，必须追加到系统日志.md：`[LOG-YYYYMMDD-NN] cognitive-reorganize | 完成认知结构重组 | L0大脑总地图.md`
  → 若重组过程中发现"某类文档反复归属不清，需要新建分类维度"：记录到 cognitive/L3_logs/todo.md（待处理：需要新建分类维度）
- **通路E（Loop 2 → Loop 1 反向）**：
