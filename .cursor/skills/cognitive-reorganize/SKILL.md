...
Step d  ⚠️【单文件询问时执行】若触发来源是对「某具体文件」的询问（用户提供了文件名或路径）：
        ...
        轴一（Loop归属）判断：
          「这个文件是关于『如何执行某类任务』的规范/指南吗？」
            → 是 = Loop1（.cursor/ 或 docs/dev_standards/，不迁入认知结构）
          「这是某个具体工作任务的输入或输出吗？」
            → 是 = Loop3（projects/ 或 work/ 等工作目录，不迁入认知结构）
...
Loop 1 — Skill体系（.cursor/ 固定路径）
  ...
  → 开发规范、基础设施手册（在 docs/dev_standards/）
  → 引用模板（若有设计价值 → 迁入对应 L1 维度知识库）
  测试：「这是关于『如何执行某类任务』的规范或指南吗？」→ 是 = Loop1

Loop 3 — 场景投射/工作产物（projects/ 或 work/）
...
扫描范围（v1.1 扩展为全工作区，按优先级）：
  0. 认知结构/ 现有状态（第零步扫描结果）
  1. work/产品定义/
  2. cognitive/L2_fragments/ 和 AI思维锻炼/ 和 AI思维问题复盘/
  3. work/research/
  4. docs/dev_standards/（Loop1 文档，不迁移，但记录）
  5. work/skill-system-design/（Loop1 设计工件，记录归属）
  6. projects/*/docs/（只看文档，不看代码）
...
Round B只需AI自己判断，以下情况才提交User：
  ✗ 两文档结论矛盾，无法从更高层原则推导谁对
  ✗ 一个文档的「意图」不清楚（User为什么写这个）
  ✗ 确实无法区分两个版本的权威性
  ✗ 需要合并两个文档（涉及内容创作决策）
```
...
Loop1 文档（开发规范/踩坑速查/Skill模板等）：
  → 若在认知结构内 → 迁出到 docs/dev_standards/ 或 .cursor/skills/*/knowledge/
  → 若已在正确位置 → 标注 ✅ 无需迁移
  → 若是「引用模板有REF-EXT价值」→ 在对应 L1 维度知识库里放一份（保持 .cursor/ 里的原本）

Loop3 文档（项目工作文档）：
  → 若在工作目录根下 → 确认归属，迁入 projects/{项目名}/
  → 若已在 projects/ 里 → 标注 ✅ 无需迁移
  → 工作产物（HTML/图片/报告输出）→ 保持原位或迁入 projects/{项目}/outputs/
...
2. **AI自主读文档判断，不问User**：UNCLEAR项应 <10%，其余都能自己判断
